...
// The swarm half of the same question. The case that mattered was a swarm
// layer at ABI 1 over a core at an ABI this host did not speak -- which,
// before the host learned 2, was every build from v0.16.0: the layer's own
// check passed, the core refused underneath, and Start returned to "No
// swarm is running" with nothing said. Core 3 stands in for that now.
test.describe('a swarm module the Lab cannot drive', () => {
  test('gives a reason whenever it is not capable, including a dv-ABI-2 core', async ({ page }) => {
    ...
    expect(byName['layer 1, core 2'].capable).toBe(true);
    expect(byName['layer 1, core 3'].problems.join(' ')).toContain('dv ABI 3');
  });
});
