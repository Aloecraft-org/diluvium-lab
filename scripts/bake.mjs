...
async function main() {
  ...
  const bundle = [
    ...
    // The About panel says "commit unknown" for a page served from a
    // checkout, because nothing there could have looked at git. Baking is
    // the one place a build step exists, so it is the one place that can
    // answer -- and a baked file is exactly the artifact that gets passed
    // around detached from the repo it came from.
    ...(commit ? [`window.DILUVIUM_LAB_COMMIT = ${JSON.stringify(commit)};`] : []),
  ...
  console.log(`  ${mb(baked.length)} total${commit ? `, commit ${commit.slice(0, 12)}` : ''}`);
}

/** The commit, or null outside a git checkout. Never a guess. */
function gitCommit() {
  try {
